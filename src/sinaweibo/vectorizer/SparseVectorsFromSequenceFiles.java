...
import sinaweibo.common.CommandLineUtil;
...
import sinaweibo.math.hadoop.stats.BasicStats;
import sinaweibo.vectorizer.collocations.llr.LLRReducer;
...
public final class SparseVectorsFromSequenceFiles extends AbstractJob {
  ...
  public int run(String[] args) throws Exception {
     ...
     */
    Option parseWeiboOpt = obuilder.withLongName("parseWeibo").withRequired(false).withDescription(
    	      "Treat each doc as weibo and filter special tokens").withShortName("pw").create();   
    
    Option topCFWordNumOpt = obuilder.withLongName("topCFWordNum").withRequired(false).withArgument(
    	      abuilder.withName("topCFWordNum").withMinimum(1).withMaximum(1).create()).withDescription(
    	      "Specify a number N and only keep top N words with highest collection frequency").withShortName("tcf").create();
    ...
    Group group = gbuilder.withName("Options").withOption(minSupportOpt).withOption(analyzerNameOpt)
        ...
        .withOption(logNormalizeOpt).withOption(parseWeiboOpt).withOption(topCFWordNumOpt)
        .create();
    try {
      ...
      boolean parseWeibo = false;
      if (cmdLine.hasOption(parseWeiboOpt)) {
    	  parseWeibo = true;
      }
      
      int topCFWordNum = -1;
      if (cmdLine.hasOption(topCFWordNumOpt)) {
    	  topCFWordNum = Integer.parseInt(cmdLine.getValue(topCFWordNumOpt).toString());
      }
      ...
      if (!processIdf) {
        DictionaryVectorizer.createTermFrequencyVectors(tokenizedPath, outputDir, tfDirName, conf, minSupport, maxNGramSize,
          minLLRValue, norm, logNormalize, reduceTasks, chunkSize, sequentialAccessOutput, namedVectors, parseWeibo, topCFWordNum);
      } else {
        DictionaryVectorizer.createTermFrequencyVectors(tokenizedPath, outputDir, tfDirName, conf, minSupport, maxNGramSize,
          minLLRValue, -1.0f, false, reduceTasks, chunkSize, sequentialAccessOutput, namedVectors, parseWeibo, topCFWordNum);
      }
      Pair<Long[], List<Path>> docFrequenciesFeatures = null;
       ...
       if (shouldPrune) {
          ...
          * Xudong Zhang:
          * 2013/12/19
          * Gen new remap dictionary for weka 
          */
         GenDictRemapForWeka(conf, docFrequenciesFeatures, minDf, maxDF, outputDir);
    ...
    return 0;
  	...
  	/*
  	 * This step is for weka, to reduce the size of the dict
  	 * Only keep the word whose DF is between minDF and maxDF.
  	 * Reallocate the index of word from 0 and keep the mapping
  	 * Assuming the remap file can be put into memory
  	 * 
  	 */
	void GenDictRemapForWeka(Configuration conf, Pair<Long[], List<Path>> docFrequenciesFeatures,
			long minDF, long maxDF, Path outputDir) throws IOException {
		
		Long featureCount = docFrequenciesFeatures.getFirst()[0];
        Long vectorCount = docFrequenciesFeatures.getFirst()[1];
		
		List<Path> dictionaryChunks = docFrequenciesFeatures.getSecond();
		FileSystem fs = FileSystem.get(conf);
		Path dictRemapPath = new Path(outputDir.toString(), TFIDFConverter.DICT_REMAP_FILE);
		SequenceFile.Writer remapWriter =
			      new SequenceFile.Writer(fs, conf, dictRemapPath, IntWritable.class, IntWritable.class);
		...
		try {		
			...
			for (Path dictionaryChunk : dictionaryChunks) {
				for (Pair<IntWritable, LongWritable> record : new SequenceFileIterable<IntWritable, LongWritable>(
						dictionaryChunk, true, conf)) {
					long df = record.getSecond().get();
					if (df < minDF ||
						(maxDF > -1 && (100.0 * df) / vectorCount > maxDF)	) {
						continue;
					}
					remapWriter.append(record.getFirst(), new IntWritable(newIdx++));
				}
			}
	    } finally {
			Closeables.closeQuietly(remapWriter);
		}
	    System.out.println("Original feature count: " + featureCount + " new feature count: " + newIdx);
	}
