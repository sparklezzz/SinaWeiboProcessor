...
import sinaweibo.util.InverseReducer;
import sinaweibo.util.LongWritableDecreasingComparator;
import sinaweibo.vectorizer.collocations.llr.CollocDriver;
import sinaweibo.vectorizer.common.PartialVectorMerger;
import sinaweibo.vectorizer.term.TFPartialVectorReducer;
import sinaweibo.vectorizer.term.TermCountCombiner;
import sinaweibo.vectorizer.term.TermCountMapper;
import sinaweibo.vectorizer.term.TermCountReducer;
...
public final class DictionaryVectorizer implements Vectorizer{
  ...
  public static final String PARSE_WEIBO = "parse.weibo";	
  ...
  private static final String DICTIONARY_SORTED_JOB_FOLDER = "wordcount-sorted";
   ...
   * @param output
   *          output directory where {@link sinaweibo.math.RandomAccessSparseVector}'s of the document
   *          are generated
  ...
  public static void createTermFrequencyVectors(Path input,
    ...
    if (maxNGramSize == 1) {
      startWordCounting(input, dictionaryJobPath, baseConf, minSupport, parseWeibo);
      if (topCFWordNum == -1) {
    	  dictionaryChunks =
    			  createDictionaryChunks(dictionaryJobPath, output, baseConf, chunkSizeInMegabytes, topCFWordNum, maxTermDimension);
      } else {    	  
    	  Path dictionarySortedJobPath = new Path(output, DICTIONARY_SORTED_JOB_FOLDER);
    	  sortWordByTopCF(dictionaryJobPath, dictionarySortedJobPath, baseConf);
    	  dictionaryChunks =
    			  createDictionaryChunks(dictionarySortedJobPath, output, baseConf, chunkSizeInMegabytes, topCFWordNum, maxTermDimension);
      }
    } else {
  ...
  private static List<Path> createDictionaryChunks(Path wordCountPath,
    ...
    try {
      ...
      for (Pair<Writable,Writable> record
        ...
        if (topCFWordNum != -1 && i == topCFWordNum) {
        	break;
        }
      }
   ...
   */
  private static void startWordCounting(Path input, Path output, Configuration baseConf, int minSupport, boolean parseWeibo)
    throws IOException, InterruptedException, ClassNotFoundException {
    ...
    conf.setBoolean(PARSE_WEIBO, parseWeibo);
  ...
  /**
   * Sort words by their collection frequency. The input documents have to be in
   * {@link SequenceFile} format
   */
  private static void sortWordByTopCF(Path input, Path output, Configuration baseConf)
    throws IOException, InterruptedException, ClassNotFoundException {
    ...
    job.setJobName("DictionaryVectorizer::SortWordByCollectionFreq: input-folder: " + input);
    job.setJarByClass(DictionaryVectorizer.class);
