...
/*
 * preprocess the keyword_title file: 
 * (1) add file type 2 to the second field
 * (2) write to sequential file
 * 
 */
public class KeywordTitlePreprocessor {
	...
	public static void process(Path inputPath, Path outputPath, Configuration baseConf) throws IOException, InterruptedException, ClassNotFoundException {
		...
		Job job = new Job(conf, className);	
		 ...
		 job.setJarByClass(KeywordTitlePreprocessor.class);//主类
		 job.setMapperClass(KeywordTitlePreprocessorMapper.class);//mapper
