...
/*
 * preprocess the keyword_user file: 
 * (1) add file type 2 to the second field
 * (2) write to sequential file
 * 
 */
public class KeywordUserPreprocessor {
	...
	public static void process(Path inputPath, Path outputPath, Configuration baseConf) throws IOException, InterruptedException, ClassNotFoundException {
		...
		job.setJarByClass(KeywordUserPreprocessor.class);//主类
		 job.setMapperClass(KeywordUserPreprocessorMapper.class);//mapper
		 job.setReducerClass(KeywordUserPreprocessorReducer.class);//reducer
