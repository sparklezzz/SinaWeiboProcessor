...
public class SequenceStringTuple2Text {

	public static class MyMapper extends Mapper<Text, StringTuple, Text, Text> {
		private final Text newVal = new Text();
		private final StringBuffer m_buffer = new StringBuffer();

		public void map(Text key, StringTuple value, Context context)
				throws IOException, InterruptedException {
			m_buffer.delete(0, m_buffer.length());

			for (String word : value.getEntries()) {
				m_buffer.append(word + " ");
			}
	...
	private static void runMapReduceJob(Configuration conf, Path path,
			Path path2) throws IOException, InterruptedException,
		...
		job.setJarByClass(SequenceStringTuple2Text.class);// 主类
