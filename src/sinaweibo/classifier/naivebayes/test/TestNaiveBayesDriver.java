...
import sinaweibo.classifier.naivebayes.PriorNaiveBayesClassifier;
...
public class TestNaiveBayesDriver extends AbstractJob {
  ...
  public int run(String[] args) throws Exception {
    ...
    addOption(buildOption("usePrior", "p", "use prior?", false, false, String.valueOf(false)));
    ...
    boolean usePrior = hasOption("usePrior");
    ...
    if (sequential) {
      ...
      if (complementary) {
    	System.out.println("Using complementary bayes classifier...");
        classifier = new ComplementaryNaiveBayesClassifier(model);
      } else if (usePrior) {
      	System.out.println("Using prior bayes classifier...");
        classifier = new PriorNaiveBayesClassifier(model);
      } else {
    	System.out.println("Using standard bayes classifier...");
      ...
      while (reader.next(key, vw)) {
    	  // modified by Zhang Xudong
    	  writer.append(new Text(key.toString()),
  ...
  private boolean runMapReduce(Map<String, List<String>> parsedArgs) throws IOException,
    ...
    boolean usePrior = parsedArgs.containsKey("--usePrior");
    testJob.getConfiguration().set(PRIOR, String.valueOf(usePrior));
  ...
  private static void analyzeResults(Map<Integer, String> labelMap,
	...
	for (Pair<Text, VectorWritable> pair : dirIterable) {
      ...
      if (bestIdx == Integer.MIN_VALUE) {	// no score, random
    	  bestIdx = (int) (Math.random() * pair.getSecond().get().size());
    	  randomCount ++;
      }
      ClassifierResult classifierResult = new ClassifierResult(labelMap.get(bestIdx), bestScore);
      analyzer.addInstance(pair.getFirst().toString().split("/")[1], classifierResult);
      //analyzer.addInstance(pair.getFirst().toString(), classifierResult);
    }
	System.out.println("Random classified: " + randomCount);
  }
}
