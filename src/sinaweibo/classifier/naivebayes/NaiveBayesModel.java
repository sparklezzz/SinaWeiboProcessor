...
import sinaweibo.math.SparseRowMatrix;
...
public class NaiveBayesModel {
  ...
  private final Vector instanceCountPerLabel;
  ...
  public NaiveBayesModel(Matrix weightMatrix,
    ...
    this.instanceCountPerLabel = instanceCountPerLabel;
  }
  ...
  public double instanceCount(int label) {
	  return instanceCountPerLabel.getQuick(label);
  }
  ...
  public static NaiveBayesModel materialize(Path output, Configuration conf) throws IOException {
    ...
    Vector instanceCountPerLabel = null;
    float alphaI;
    ...
    try {
      ...
      instanceCountPerLabel = new DenseVector(VectorWritable.readVector(in));
    } finally {
      Closeables.closeQuietly(in);
    }
    NaiveBayesModel model = new NaiveBayesModel(weightsPerLabelAndFeature, weightsPerFeature, weightsPerLabel,
        perLabelThetaNormalizer, instanceCountPerLabel, alphaI);
    model.validate();
    return model;
  }
  ...
  public void serialize(Path output, Configuration conf) throws IOException {
    ...
    try {
      ...
      VectorWritable.writeVector(out, instanceCountPerLabel);
    } finally {
  ...
  public void validate() {
    ...
    Preconditions.checkArgument(instanceCountPerLabel != null, "the instance count vector have to be defined");
    Preconditions.checkArgument(instanceCountPerLabel.getNumNondefaultElements() > 0,
        "the labels of instance count vector have to be greater than 0!");
